public class Solution {
    // Rectangle unit count prefix sum
    private int[][] ruc;
    private int height;
    private int width;

    // Utility for fast region sum
    private int unitsInRectangle(int r0, int c0, int r1, int c1) {
        return ruc[r1][c1] - ruc[r0][c1] - ruc[r1][c0] + ruc[r0][c0];
    }

    // Gets min area covering all 1's in rectangle; returns 0 if there's no '1'
    ...
    // Find min area covering all 1s with 2 rectangles (vertical/horizontal split)
    ...
    // Find min area covering all 1s with 3 rectangles (all 2-cut partitions)
    private int minSum3(boolean firstSplitVertical, boolean takeLower, boolean secondSplitVertical) {
        int min = Integer.MAX_VALUE;
